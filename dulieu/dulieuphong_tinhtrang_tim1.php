...
if (isset($_POST['id_toanha_tinhtrang12']) && isset($_POST['tang_p_tinhtrang12'])) {
	include 'conn.php';
	$selecet_phong = mysqli_query($con, "SELECT DISTINCT toa_nha.id_toanha, toa_nha.ten_toa_nha, phong.idphong, phong.ma_phong, phong.stt_tang, loai_phong.id_loaiphong, loai_phong.ten_loai_phong, loai_phong.sl_nguoi_o FROM phong, toa_nha, loai_phong, sinh_vien WHERE phong.xoa=0 and toa_nha.id_toanha='$_POST[id_toanha_tinhtrang12]'  and phong.stt_tang ='$_POST[tang_p_tinhtrang12]'AND sinh_vien.gioi_tinh= toa_nha.loai_toa_nha AND toa_nha.xoa=0 AND phong.id_toanha=toa_nha.id_toanha AND loai_phong.xoa=0 AND phong.id_loaiphong=loai_phong.id_loaiphong ORDER BY toa_nha.ten_toa_nha, phong.stt_tang, phong.ma_phong ");
	if (!mysqli_num_rows($selecet_phong)) {
		echo "<div style='text-align: center;'> Chưa có dữ liệu</div>";
	} else {
	?>
...
if (isset($_POST['chon_phong_tinhtrang12']) ) {
	include 'conn.php';
	$selecet_phong = mysqli_query($con, "SELECT DISTINCT toa_nha.id_toanha, toa_nha.ten_toa_nha, phong.idphong, phong.ma_phong, phong.stt_tang, loai_phong.id_loaiphong, loai_phong.ten_loai_phong, loai_phong.sl_nguoi_o FROM phong, toa_nha, loai_phong, sinh_vien WHERE phong.xoa=0   and phong.idphong ='$_POST[chon_phong_tinhtrang12]'AND sinh_vien.gioi_tinh= toa_nha.loai_toa_nha AND toa_nha.xoa=0 AND phong.id_toanha=toa_nha.id_toanha AND loai_phong.xoa=0 AND phong.id_loaiphong=loai_phong.id_loaiphong ORDER BY toa_nha.ten_toa_nha, phong.stt_tang, phong.ma_phong ");
	if (!mysqli_num_rows($selecet_phong)) {
		echo "<div style='text-align: center;'> Chưa có dữ liệu</div>";
	} else {
	?>
