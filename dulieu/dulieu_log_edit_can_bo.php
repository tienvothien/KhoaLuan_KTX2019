<?php
include 'conn.php';
	$selecet_khoa = mysqli_query($con, "SELECT * FROM log_sua_dl WHERE log_sua_dl.bangsua='can_bo'  ORDER BY ngaysua DESC");
		...
		<tbody>
			<?php
				...
				while ($row = mysqli_fetch_array($selecet_khoa)) {
					...
					// thông tin lớp
					$tt_can_bo = mysqli_fetch_array(mysqli_query($con, "SELECT  can_bo.ma_can_bo, can_bo.ho_can_bo, can_bo.ten_can_bo FROM can_bo WHERE can_bo.id_canbo='$row[iddulieu]' "));
					...
					<tr>
						...
						<td class='canhgiua chuinhoa'>$tt_can_bo[ma_can_bo]</td>
						<td class='canhgiua chuinthuong'>$tt_can_bo[ho_can_bo] $tt_can_bo[ten_can_bo] </td>
