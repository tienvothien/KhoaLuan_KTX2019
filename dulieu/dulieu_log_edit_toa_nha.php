<?php
include 'conn.php';
	$selecet_khoa = mysqli_query($con, "SELECT * FROM log_sua_dl WHERE log_sua_dl.bangsua='toa_nha' ");
		...
		<tbody>
			<?php
				...
				while ($row = mysqli_fetch_array($selecet_khoa)) {
					...
					// thông tin lớp
					$tt_toa_nha = mysqli_fetch_array(mysqli_query($con, "SELECT  toa_nha.ma_toa_nha,toa_nha.ten_toa_nha FROM toa_nha WHERE toa_nha.id_toanha='$row[iddulieu]'"));
					...
					<tr>
						...
						<td class='canhgiua chuinhoa'>$tt_toa_nha[ma_toa_nha]</td>
						<td class='canhgiua chuinthuong'>$tt_toa_nha[ten_toa_nha] </td>
