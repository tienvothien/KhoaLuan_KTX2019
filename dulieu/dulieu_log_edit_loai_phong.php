<?php
include 'conn.php';
	$selecet_khoa = mysqli_query($con, "SELECT * FROM log_sua_dl WHERE log_sua_dl.bangsua='loai_phong'  ORDER BY ngaysua DESC");
		...
		<tbody>
			<?php
				...
				while ($row = mysqli_fetch_array($selecet_khoa)) {
					...
					// thông tin lớp
					$tt_loai_phong = mysqli_fetch_array(mysqli_query($con, "SELECT  loai_phong.ma_loai_phong, loai_phong.ten_loai_phong FROM loai_phong WHERE loai_phong.id_loaiphong='$row[iddulieu]' "));
					...
					<tr>
						...
						<td class='canhgiua chuinhoa'>$tt_loai_phong[ma_loai_phong]</td>
						<td class='canhgiua chuinthuong'>$tt_loai_phong[ten_loai_phong] </td>
